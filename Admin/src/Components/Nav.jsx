...
const Nav = () => {
  ...
  const logout = async () => {
    try {
      ...
      console.log("error from logout admin", err);
    }
  };
  return (
    <div className="w-[100vw] h-[70px] bg-[#dcdbdbf8] z-10 fixed top-0 flex items-center justify-between px-[30px] overflow-x-hidden shadow-md shadow-black">
      ...
      <button
        className="text-[15px] hover:border-[2px] border[#89daea] cursor-pointer bg-[#000000ca] py-[10px] rounded-2xl text-white px-[20px]"
        onClick={logout}
      >
        LogOut
      </button>
